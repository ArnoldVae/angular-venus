...
define([
    ...
    angular.module('backend-mocks', ['ngMockE2E'])
        ...
        .run(function ($httpBackend,ApiPath) {
            ...
            // });
            //蓝票开具申请-判断复选框是否可以勾选
            // $httpBackend.whenPOST(ApiPath.api.buleIsTrueSubmit).respond(function (method, url, data) {
            //     console.log(data);
            //     return[200,jsonDB.buleIsTrueSubmit];
            // });
            //蓝票开具申请-勾选提交
            ...
            // });
            //蓝票开具申请-change信息
            // $httpBackend.whenPOST(ApiPath.api.blueChangeInfomation).respond(function (method, url, data) {
            //     console.log(data);
            //     return[200,jsonDB.blueChangeInfomation];
            // });
            //批量蓝票开具申请-查询
            ...
            // });
            //基础设置--银行账户维护--银行商户信息新增deleteBankMerchant/newBankMerchant
            ...
            ////到款确认-银行信息
            //$httpBackend.whenPOST(ApiPath.api.confirmBank).respond(function (method, url, data) {
            //    console.log(data);
            ...
            //    console.log(data);
            //    return[200,jsonDB.AccountSubmit];
            //});
            ////自动到款确认
            //$httpBackend.whenPOST(ApiPath.api.paymentAndSpayCheck).respond(function (method, url, data) {
            //    console.log(data);
            //    return[200,jsonDB.paymentAndSpayCheck];
            //});

            //付赔款-查询（调通）
            // $httpBackend.whenPOST(config.backend.ip+config.backend.claimQuery).respond(function (method, url, data) {
            //     console.log(data);
            //     return[200,jsonDB.ClaimQuery];
            // });
            // 付赔款-查询（调通）
            ...
            // });
            //收付场景--查询
            $httpBackend.whenPOST(config.backend.ip+config.backend.sceneSearch).respond(function (method, url, data) {
                console.log(data);
                return[200,jsonDB.SceneSearch];
            });
            //收付场景--新增
            $httpBackend.whenPOST(config.backend.ip+config.backend.addScene).respond(function (method, url, data) {
                console.log(data);
                return[200,jsonDB.AddScene];
            });
            //收付场景--修改
            $httpBackend.whenPOST(config.backend.ip+config.backend.modifyScene).respond(function (method, url, data) {
                console.log(data);
                return[200,jsonDB.ModifyScene];
            });
            //收付场景--删除
            $httpBackend.whenPOST(config.backend.ip+config.backend.deleteScene).respond(function (method, url, data) {
                console.log(data);
                return [200, jsonDB.DeleteScene];
            });
            //收付场景--修改确认
            $httpBackend.whenPOST(config.backend.ip+config.backend.updateSceneSave).respond(function (method, url, data) {
                console.log(data);
                return [200, jsonDB.UpdateSceneSave];
            });
            //收付类型--修改确认
            $httpBackend.whenPOST(config.backend.ip+config.backend.updateTypeSave).respond(function (method, url, data) {
                console.log(data);
                return [200, jsonDB.UpdateTypeSave];
            });

            //收付类型--查询
            $httpBackend.whenPOST(config.backend.ip+config.backend.typeQuery).respond(function (method, url, data) {
                console.log(data);
                return [200, jsonDB.TypeQuery];
            });
            //收付类型--新增
            $httpBackend.whenPOST(config.backend.ip+config.backend.addType).respond(function (method, url, data) {
                console.log(data);
            ...
            //收付类型--删除
            $httpBackend.whenPOST(config.backend.ip+config.backend.deleteType).respond(function (method, url, data) {
                console.log(data);
                return[200,jsonDB.DeleteType];
            });
            //核算项树查询(调通)
            $httpBackend.whenPOST(config.backend.ip+config.backend.accountItemGroup).respond(function (method, url, data) {
                console.log(data);
                return [200, jsonDB.accountItemGroupData];
            });
            // 辅助核算项维护树形菜单查询(调通)
            $httpBackend.whenPOST(config.backend.ip+config.backend.queryAccount).respond(function (method, url, data) {
                console.log(data);
                return [200, jsonDB.QueryAccount];
            });
            // 会计科目设置-2辅助核算项-全部保存(新加)
            $httpBackend.whenPOST(config.backend.ip+config.backend.saveAccountAll).respond(function (method, url, data) {
                console.log(data);
                return [200, jsonDB.saveAccountAll];
            });
            // 核算项保存(调通)
            $httpBackend.whenPOST(config.backend.ip+config.backend.saveCenterAccount).respond(function (method, url, data) {
                console.log(data);
                return [200, jsonDB.SaveCenterAccount];
            });
            // 核算项删除(调通)
            $httpBackend.whenPOST(config.backend.ip+config.backend.delCenterInf).respond(function (method, url, data) {
                console.log(data);
                return [200, jsonDB.DelCenterInf];
            });

            // 收付原因查询  (完成)
            $httpBackend.whenPOST(config.backend.ip+config.backend.payReason).respond(function (method, url, data) {
            ...
            //});
            // 收付原因新增  (完成)
            $httpBackend.whenPOST(config.backend.ip+config.backend.addPayReason).respond(function (method, url, data) {
                return [200, jsonDB.AddPayReason];
            });
            // 收付原因修改  (完成)
            $httpBackend.whenPOST(config.backend.ip+config.backend.updatePayReason).respond(function (method, url, data) {
                return [200, jsonDB.AddPayReason];
            });
            // 收付原因删除 (完成)
            $httpBackend.whenPOST(config.backend.ip+config.backend.deletePayReason).respond(function (method, url, data) {
           ...
           // });
            ...
            //});
            //收付方式查询(调通)
            $httpBackend.whenPOST(config.backend.ip+config.backend.searchPaymentData).respond(function (method, url, data) {
                console.log(data);
